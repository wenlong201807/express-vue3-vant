...
| 8 | `e2e/play-record.e2e.spec.ts` | playwright e2e | 真实前后端：播放 17s 三指标经 HTTP 校验；hidden 补报且无定时器上报；路由跳转补报；列表三态；续播反显（currentTime=服务端 position）；.vjs-play-control/button.vjs-playback-rate 存在 | `npm run test:e2e` | 本机已装 Google Chrome（config channel:'chrome'）；3000/5173 端口空闲（webServer 自动起停） | `5 passed` |
